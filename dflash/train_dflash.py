...
Creates a new DFlash model initialized with Alpamayo's vocabulary
and trains it using pre-computed distillation data.

Features:
- Creates a NEW DFlashDraftModel from scratch
...
# Default layer selection for training (subset of stored layers)
...
class OfflineDistillationDataset(Dataset):
    ...
    Supports selecting a subset of stored layers for training flexibility.
    """
    ...
    def __init__(
        ...
        # Get stored layer IDs from metadata
        self.stored_layer_ids = self.metadata.get("target_layer_ids", DEFAULT_TARGET_LAYER_IDS)

        # Determine which layers to use for training
        if target_layer_ids is None:
            target_layer_ids = DEFAULT_TARGET_LAYER_IDS

        # Validate that target layers are a subset of stored layers
        for layer_id in target_layer_ids:
            if layer_id not in self.stored_layer_ids:
                raise ValueError(
                    f"Layer {layer_id} not in stored layers {self.stored_layer_ids}. "
                    f"Target layers must be a subset of stored layers."
                )

        self.target_layer_ids = target_layer_ids

        # Compute indices for extracting target layers from concatenated hidden states
        self.layer_indices = []
        for layer_id in self.target_layer_ids:
            idx = self.stored_layer_ids.index(layer_id)
            start = idx * hidden_dim
            end = (idx + 1) * hidden_dim
            self.layer_indices.append((start, end))

        # Load resolution scale from metadata (default 1.0 for backward compatibility)
        self.resolution_scale = self.metadata.get("resolution_scale", 1.0)

        log_rank = rank if rank >= 0 else 0
        logger.info(f"[Rank {log_rank}] Stored layers: {self.stored_layer_ids}")
        logger.info(f"[Rank {log_rank}] Using layers: {self.target_layer_ids}")
        logger.info(f"[Rank {log_rank}] Resolution scale: {self.resolution_scale}")
...
class ScratchTrainer:
    ...
    def __init__(
        ...
        # Pre-compute prefix weights (cached for efficiency)
        T = block_size - 1  # Labels are for positions 1 to block_size
        step = torch.arange(T, device=self.device, dtype=torch.float32)
        self._prefix_weights = torch.exp(-step / prefix_weight_gamma)
        self._prefix_weights = self._prefix_weights / self._prefix_weights.mean()
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        with torch.autocast("cuda", dtype=torch.bfloat16):
            ...
            # Prefix-weighted cross-entropy (use cached weights)
            ...
            if valid_count > 0:
                ce_loss = (ce_per_token * self._prefix_weights[None, :] * valid_mask).sum() / valid_count
            else:
    ...
    def save_checkpoint(self, path: str | Path, epoch: int, val_loss: float | None = None, resolution_scale: float = 1.0):
        ...
        self.model_without_ddp.config.resolution_scale = resolution_scale
...
def main():
    ...
    parser.add_argument("--target-layers", type=str, default=None,
                        help="Comma-separated layer IDs to use from stored data (e.g., '24,30,31,32,34'). "
                             "Must be a subset of layers stored during data generation. "
                             "Default: [24,30,31,32,34]")
    # Performance
    ...
    # Expand ~ in paths
    args.target_model = os.path.expanduser(args.target_model)
    ...
    if is_main_process():
        ...
        logger.info(f"Target layers: {target_layer_ids}")

    # Create output directory
    timestamp = datetime.now().strftime("%m%d_%H%M%S")
    exp_name = f"dflash_train_{timestamp}"
    ...
    # Set target_layer_ids on config to ensure fc layer has correct input dimension
    # This must match the layers stored in the training data
    ...
    # Get resolution scale from training data metadata
    resolution_scale = train_dataset.resolution_scale
    ...
    for epoch in range(1, args.num_epochs + 1):
        ...
        if val_loss < best_val_loss:
            ...
            trainer.save_checkpoint(output_dir / "best", epoch, val_loss, resolution_scale)
            if is_main_process():
    ...
    # Save final
    trainer.save_checkpoint(output_dir / "final", args.num_epochs, val_loss, resolution_scale)
