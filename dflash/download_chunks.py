...
def get_files_to_download(chunks_needed: list[int]) -> list[tuple[str, int]]:
    ...
    api = HfApi()
    files = list(api.list_repo_tree(REPO_ID, repo_type='dataset', revision=REVISION, recursive=True))
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Number of chunks to download (downloads chunks 0..N-1)",
    )
    parser.add_argument(
        "--chunks-file",
        type=str,
        default=None,
        help="JSON file containing a list of specific chunk indices to download",
    )
    parser.add_argument(
        "--workers",
        type=int,
    ...
    args.output_log = os.path.expanduser(args.output_log)

    Path(args.cache_dir).mkdir(parents=True, exist_ok=True)

    # Step 1: Determine chunks to download
    if args.chunks_file:
        chunks_file = os.path.expanduser(args.chunks_file)
        with open(chunks_file) as f:
            chunks_needed = json.load(f)
        logger.info(f"Loaded {len(chunks_needed)} chunk indices from {chunks_file}")
    elif args.num_chunks:
        chunks_needed = get_chunks_to_download(args.num_chunks)
    else:
        parser.error("Must specify either --num-chunks or --chunks-file")
