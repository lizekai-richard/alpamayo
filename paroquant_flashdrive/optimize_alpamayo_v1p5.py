#!/usr/bin/env python3
"""Layerwise ParoQuant optimization for Alpamayo1.5.
...
Usage:
    ...
    python paroquant/optimize_alpamayo.py \\
        --model /data/scratch/zekaili/Alpamayo-1.5-10B \\
        --target vlm \\
...
# Upstream ParoQuant repo root (../paroquant/): must contain package paroquant/
sys.path.append(str(Path(__file__).resolve().parents[1] / "paroquant"))
# src/ contains the alpamayo1_5 package
...
def get_alpamayo_blocks(model, target: str) -> nn.ModuleList:
    """Return the transformer layer list for the given Alpamayo1.5 component."""
    if target == "expert":
...
class Config:
    # Huggingface model path or local path to Alpamayo1.5.
    model: str
...
def main():
    ...
    # Load Alpamayo1.5 model
    from alpamayo_r1.models.alpamayo_r1p5 import Alpamayo1_5

    logger.info(f"Loading Alpamayo1.5 from {args.model}...")
