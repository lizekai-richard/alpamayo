...
def main():
    ...
    n = len(all_timing)
    log.info(f"\n{'='*60}")
    log.info(f"AGGREGATE RESULTS — DFlash + Streaming (torch.compile + speculative decoding + KV reuse) ({n} steps)")
    log.info("=" * 60)
    if all_timing:
        def avg(key):
    ...
    out_file = os.path.join(
        run_dir,
        f"dflash_streaming_K{args.num_traj_samples}_d{args.diffusion_steps}_{len(clip_ids)}clips.json",
    )
